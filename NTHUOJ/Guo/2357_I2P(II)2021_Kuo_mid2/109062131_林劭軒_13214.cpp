...
void Banker::Pay(Human *human)
{
    ...
    //     cdbg << "Banker pay " << human->name << ' ' << bankerToPay << endl;
}
...
void Casino::TwentyOne()
{
    ...
    // for (int i = 0; i < player_num; i++)
    // {
    //     cdbg << player[i]->name << ' ' << player[i]->money << endl;
    // }
    ...
    banker->Draw();
    //     cdbg << "Banker cards  " << banker->cards << endl;

    for (int i = 0; i < player_num; i++)
    {
        if (record[i] == 1)
        {
            // cdbg << "Banker Win: " << banker->Win(player[i]) << endl;
            if (banker->Win(player[i]) == 0)
            {
                banker->Pay(player[i]);
                // cdbg << bankerToPay << ' ' << server->skill << endl;
...
void Casino::EndDay()
{
    ...
    for (int i = 0; i < player_num; i++)
        if (player[i])
        {
            //     cout << "Deleting " << player[i]->name << endl;
            delete dynamic_cast<Player *>(player[i]);
            player[i] = nullptr;
        }
