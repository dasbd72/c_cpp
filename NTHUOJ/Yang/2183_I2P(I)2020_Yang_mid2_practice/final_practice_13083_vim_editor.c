...
int main() {
    ...
    while (T--) {
        scanf("%s", s);
        // Initialize
        char** ans = malloc((MAX_LINES + 10) * sizeof(char*));
        int* size = malloc((MAX_LINES + 10) * sizeof(int));
        int* capacity = malloc((MAX_LINES + 10) * sizeof(int));
            ...
            if (mode == NormalMode) {
                ...
                if (s[i] == 'E' && s[i + 1] == 'S' && s[i + 2] == 'C') {
                    ...
                    if (idx * repeat + size[cursorY] >= capacity[cursorY]) {
                        ...
                        char *copied = malloc(capacity[cursorY] * sizeof(char));
